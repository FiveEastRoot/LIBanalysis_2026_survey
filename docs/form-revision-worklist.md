...
- 개인정보 취급위탁 동의 여부를 먼저 선택하고, 동의 시 전화번호 입력창을 표시합니다.
- 개인정보 취급위탁에 동의하지 않는 경우 전화번호 없이 바로 다음 문항으로 이동하고, 같은 브라우저에서는 localStorage로 재제출을 제한합니다.
- 개인정보 취급 안내는 접기/펼치기 방식입니다.
- 전체 진행도는 export 대상 필드 기준으로 계산합니다.
- 영역별 빠른 이동은 해당 영역의 첫 미완료 문항으로 이동합니다.
- 진행도 인디케이터에서 공통 만족도는 문항번호 규칙에 따라 세분화하되, Q7 비용 혜택과 Q8 투자 필요성은 `비용 혜택 & 투자 필요성`으로 통합 표시합니다.
...
- 한 페이지에 직접 입력칸이 여러 개 있고 일부만 입력한 경우, 다음 이동 시 나머지 직접 입력칸은 `0`으로 자동 입력합니다.
