...
| 관리자 페이지 | 대시보드 로그인 이후 관리자 뷰에서만 다운로드/백업 가능 |
| 일반 직원 페이지 | 현황 조회 중심, 원자료 다운로드 불가 |
...
flowchart TD
    ...
    E --> I["dashboard_users<br/>대시보드 권한"]
    ...
    K --> M["관리자 뷰"]
    M --> M1["조사 현황 조회"]
    M --> M2["Analysis CSV export"]
    M --> M3["PII CSV export"]
    M --> M4["Google Sheets 백업"]
    M --> M5["운영 로그 확인"]
...
flowchart TD
    ...
    B -->|관리자| D["관리자 뷰"]
    D --> D1["조사 현황 조회"]
    D --> D2["Analysis CSV 다운로드"]
    D --> D3["PII CSV 다운로드"]
    D --> D4["Google Sheets 백업 실행"]
    D --> D5["운영 로그 확인"]
...
| Analysis CSV export | 분석용 응답만 CSV로 다운로드 | 관리자 | `admin_export_log` |
| PII CSV export | 암호화된 개인정보 파일 다운로드 | 관리자 | `admin_export_log` |
| Google Sheets 백업 | 현재 시트 구조와 같은 탭으로 백업 | 관리자 | `admin_export_log` |
...
5. 관리자 대시보드 권한 모델과 export UI 정의
6. Analysis CSV / PII CSV export 규격 확정
7. Google Sheets 백업 규격 확정
8. 분석 대시보드 입력 구조와 Supabase view/export 매핑
